...
Generation :: Generation( int n_individuals, vector<double> x_cities, vector<double> y_cities){
...
void Generation :: Sort(){
	...
	for( int i=0; i<m_Nindividuals; i++){
		bool same=false;							//the bool "same" is useful in case two or more paths share the same length
...
void Generation :: stats( string namefile ){
	...
	double sum=0.;
	int k=0;
	for( int i=0; i<m_Nindividuals/2; i++){						//path length averaged on the better half of the individuals
